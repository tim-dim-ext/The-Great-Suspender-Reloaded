...
export var gsUtils = {
  ...
  isProtectedPinnedTab: async function (tab) {
    var dontSuspendPinned = await gsStorage.getOption(gsStorage.IGNORE_PINNED);
    return dontSuspendPinned && tab.pinned;
  },

  isProtectedAudibleTab: async function (tab) {
    var dontSuspendAudible = await gsStorage.getOption(gsStorage.IGNORE_AUDIO);
    return dontSuspendAudible && tab.audible;
  },

  isProtectedActiveTab: async function (tab) {
    var dontSuspendActiveTabs = await gsStorage.getOption(
  ...
  isSuspendedUrl: function (url, looseMatching) {
    if (looseMatching) {
      return url.indexOf("html/suspended.html") > 0;
    } else {
      return url.indexOf(chrome.runtime.getURL("html/suspended.html")) === 0;
    }
  },

  shouldSuspendDiscardedTabs: async function () {
    var suspendInPlaceOfDiscard = await gsStorage.getOption(
      gsStorage.SUSPEND_IN_PLACE_OF_DISCARD
    );
    var discardInPlaceOfSuspend = await gsStorage.getOption(
  ...
  checkWhiteList: function (url, whitelist) {
  ...
  removeFromWhitelist: async function (url) {
    var oldWhitelistString = await gsStorage.getOption(gsStorage.WHITELIST) || "",
  ...
  saveToWhitelist: async function (newString) {
    var oldWhitelistString = await gsStorage.getOption(gsStorage.WHITELIST) || "";
  ...
  generateSuspendedUrl: function (url, title, scrollPos) {
    ...
    return chrome.runtime.getURL("html/suspended.html" + args);
  },
  ...
  isCharging: function(tab) {
    return new Promise(resolve => {
      gsMessages.sendRequestBatteryToContentScript(tab.id, (error, tabInfo) =>
          resolve(tabInfo && tabInfo.isCharging)
      );
    });
  },
    ...
    chrome.tabs.query({}, function (tabs) {
      tabs.forEach(async function (tab) {
        if (chrome.runtime.lastError) {
          gsUtils.error("Error while trying to know is tab is currently focused", tab, chrome.runtime.lastError);
        }
        ...
        if (gsUtils.isSuspendedTab(tab)) {
          ...
          if (
            ...
            chrome.runtime.sendMessage({function: 'unsuspendTab', tab: tab}, function () {
              if (chrome.runtime.lastError) {
                gsUtils.error("Error while trying to unsuspend tab for tab", tab, chrome.runtime.lastError);
              }
            });
            return;
          }
          ...
          if (updateTheme || updatePreviewMode) {
            ...
            if (updatePreviewMode) {
              const previewMode = await gsStorage.getOption(
                gsStorage.SCREEN_CAPTURE
              );
        ...
        const isCharging = await gsUtils.isCharging(tab);
