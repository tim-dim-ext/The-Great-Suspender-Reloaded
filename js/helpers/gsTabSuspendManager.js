/*global html2canvas, domtoimage, gsFavicon, gsMessages, gsStorage, gsUtils, gsChrome, gsIndexedDb, gsTabDiscardManager, gsTabCheckManager, GsTabQueue */
...
export var gsTabSuspendManager = (function () {
  "use strict";
  ...
  function initAsPromised() {
    return new Promise(async function (resolve) {
      ...
      const forceScreenCapture = await gsStorage.getOption(
  ...
  async function queueTabForSuspension(tab, forceLevel) {
  ...
  async function performSuspension(
    ...
    const discardInPlaceOfSuspend = await gsStorage.getOption(
  ...
  function clearAutoSuspendTimerForTabId(tabId) {
    return tgs.clearAutoSuspendTimerForTabId(tabId);
  }
  ...
  async function checkTabEligibilityForSuspension(tab, forceLevel) {
    ...
    if (forceLevel >= 2) {
      ...
      if (
        ...
        await gsUtils.checkWhiteList(tab.url, whitelist) ||
  ...
  async function requestGeneratePreviewImage(tab) {
    ...
    const useAlternateScreenCaptureLib = await gsStorage.getOption(
      gsStorage.USE_ALT_SCREEN_CAPTURE_LIB
    );
    const screenCaptureLib = useAlternateScreenCaptureLib
      ? "js/helpers/dom-to-image.js"
      : "js/helpers/html2canvas.min.js";
    gsUtils.log(
      tab.id,
      QUEUE_ID,
    ...
    gsMessages.executeScriptOnTab(tab.id, screenCaptureLib, (error) => {
      ...
      gsMessages.executeCodeOnTab(
        tab.id,
        generatePreviewImageCanvasViaContentScript,
