...
export var gsStorage = {
  ...
  initSettingsAsPromised: function() {
    return new Promise(function(resolve) {
      ...
      chrome.storage.local.get(['gsSettings'], function(localStorage) {
      chrome.storage.sync.get(defaultKeys, function(syncedSettings) {
        ...
        try {
          rawLocalSettings = JSON.parse(localStorage['gsSettings'] || false);
        } catch (e) {
          gsUtils.error(
            'gsStorage',
            'Failed to parse gsSettings: ',
            localStorage['gsSettings']
          );
        }
        if (!rawLocalSettings) {
          rawLocalSettings = {};
  ...
  getSettings: async function() {
    var settings;
    try {
      settings = JSON.parse((await chrome.storage.local.get('gsSettings'))['gsSettings'])|| false;
    } catch (e) {
      chrome.storage.local.get('gsSettings', (d) => {
        gsUtils.error(
            'gsStorage',
            'Failed to parse gsSettings: ',
            settings
  ...
  fetchLastVersion: function() {
    return new Promise(resolve => {
      chrome.storage.local.get(gsStorage.APP_VERSION, (d) => {
        try {
          var version = d[gsStorage.APP_VERSION];
          if (version) {
  ...
  setLastVersion: function(newVersion) {
    try {
      var obj = {};
      obj[gsStorage.APP_VERSION] = JSON.stringify(newVersion);
  ...
  fetchNoticeVersion: function() {
    return new Promise(resolve => {
      chrome.storage.local.get([gsStorage.LAST_NOTICE]).then((result) => {
        var lastNoticeVersion;
        try {
          lastNoticeVersion = JSON.parse(
            result[gsStorage.LAST_NOTICE]
          );
        } catch (e) {
          gsUtils.error(
            'gsStorage',
  ...
  setNoticeVersion: function(newVersion) {
    try {
      var obj = {};
      obj[gsStorage.LAST_NOTICE] = JSON.stringify(newVersion);
      chrome.storage.local.set(obj);
    } catch (e) {
      gsUtils.error(
        'gsStorage',
  ...
  fetchLastExtensionRecoveryTimestamp: function() {
    return new Promise(resolve => {
      chrome.storage.local.get([gsStorage.LAST_EXTENSION_RECOVERY]).then((result) => {
  ...
  setLastExtensionRecoveryTimestamp: function(extensionRecoveryTimestamp) {
    try {
      var obj = {};
      obj[gsStorage.LAST_EXTENSION_RECOVERY] = JSON.stringify(extensionRecoveryTimestamp);
      chrome.storage.local.set(obj);
    } catch (e) {
      gsUtils.error(
        'gsStorage',
  ...
  fetchSessionMetrics: function() {
    return new Promise(resolve => {
      chrome.storage.local.get([gsStorage.SM_SESSION_METRICS]).then((result) => {
        var sessionMetrics = {};

        try {
          sessionMetrics = JSON.parse(
            result[gsStorage.SM_SESSION_METRICS]
          );
        } catch (e) {
          gsUtils.error(
            'gsStorage',
  ...
  setSessionMetrics: function(sessionMetrics) {
    try {
      ...
      var obj = {};
      obj[gsStorage.SM_SESSION_METRICS] = JSON.stringify(sessionMetrics);
      chrome.storage.local.set(obj);
