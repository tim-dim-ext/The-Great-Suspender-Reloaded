...
export var gsSession = (function () {
  ...
  async function initAsPromised() {
    updateUrl = chrome.runtime.getURL("html/update.html");
    updatedUrl = chrome.runtime.getURL("html/updated.html");
  ...
  async function handleNormalStartup(currentSessionTabs, curVersion) {
    ...
    if (shouldRecoverTabs) {
      const lastExtensionRecoveryTimestamp =
       await gsStorage.fetchLastExtensionRecoveryTimestamp();
      ...
      if (!hasCrashedRecently) {
        ...
        const recoveryUrl = chrome.runtime.getURL("html/recovery.html");
  ...
  async function handleNewInstall(curVersion) {
    ...
    if (
      ...
      const optionsUrl = chrome.runtime.getURL("html/options.html?firstTime");
  ...
  async function handleUpdate(currentSessionTabs, curVersion, lastVersion) {
    ...
    if (shouldRecoverTabs) {
      ...
      has_updated_views = false;
      await chrome.tabs.query({}, (tabs) => {
        for (const tab of tabs) {
          if (tab.url.indexOf(chrome.runtime.getURL("html/updated.html")) === 0) {
            has_updated_views = true;
            chrome.tabs.sendMessage(tab.id, {action: "toggleUpdated"}, function() {
              if (chrome.runtime.lastError) {
                gsUtils.error("Error while toggling updated tab");
              }
            });
          }
        }
      ...
      if (has_updated_views) {
        gsUtils.removeTabsByUrlAsPromised(updatedUrl);
        gsChrome.tabsCreate({ url: updatedUrl });
      }
    } else {
      updated = true;
  ...
  async function restoreSessionWindow(
    ...
    const restoringUrl = chrome.runtime.getURL("html/restoring-window.html");
  ...
  async function createNewTabFromSessionTab(
    ...
    await chrome.tabs.query({}, (tabs) => {
      for (const tab of tabs) {
        if (tab.url.indexOf(chrome.runtime.getURL("html/recovery.html")) === 0) {
          chrome.tabs.sendMessage(tab.id, {action: "removeTabFromList"}, function() {
            if (chrome.runtime.lastError) {
              gsUtils.error("Error while removing recovery tab from list");
            }
          });
        }
      }
  ...
  async function updateSessionMetrics(reset) {
    ...
    if (reset) {
      ...
      sessionMetrics = await gsStorage.fetchSessionMetrics();
    }
