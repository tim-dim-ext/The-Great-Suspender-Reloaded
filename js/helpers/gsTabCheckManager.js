/*global chrome, localStorage, gsStorage, gsSession, gsMessages, gsUtils, gsTabDiscardManager, gsChrome, GsTabQueue, gsSuspendedTab */
...
export var gsTabCheckManager = (function() {
  'use strict';
  ...
  async function checkSuspendedTab(
    ...
    // Make sure tab is registered as a 'view' of the extension
      ...
      if (!_tab) {
        ...
        if (!executionProps.resuspended) {
          resuspendSuspendedTab(tab, function(resuspendOk){
            if (resuspendOk) {
              requeue(DEFAULT_TAB_CHECK_REQUEUE_DELAY, {
                resuspended: true,
                refetchTab: true,
    ...
    const isActiveTab = tgs.isCurrentActiveTab(tab);
    ...
    let tabSessionId = -1;
    const sessionId = await chrome.tabs.sendMessage(tab.id, {action: "getSessionId"});
    if (chrome.runtime.lastError) {
      gsUtils.error("Error while getting session ID");
    } else {
      tabSessionId = sessionId;
    }

    const tabSessionOk = tabSessionId === gsSession.getSessionId();
    const tabBasicsOk = ensureSuspendedTabTitleAndFaviconSet(tab);
    const suspendedTabVisible = await ensureSuspendedTabVisible(tab);
    const tabVisibleOk =
      attemptDiscarding || suspendedTabVisible;
    ...
    if (!tabChecksOk) {
      ...
      try {
        ...
        await gsSuspendedTab.initTab(tab, { quickInit });
        reinitialised = true;
      } catch (e) {
        gsUtils.log(
          tab.id,
  ...
  async function resuspendSuspendedTab(tab) {
    ...
    if (_tab) {
      ...
      gsUtils.log(tab.id, "tab does not exists  in resuspendSuspendedTab", tab);
    }
  ...
  async function ensureSuspendedTabVisible(tab) {
    if (!tab) {
      return false;
    }

    await chrome.tabs.sendMessage(tab.id, {action: "isBodyVisible"}, function(isVisible) {
      if (chrome.runtime.lastError) {
        gsUtils.error("Error while checking if body of suspended tab is visible");
      } else {
        return isVisible;
      }
    });
